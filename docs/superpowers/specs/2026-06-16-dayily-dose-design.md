...
| 维度 | 决定 |
|---|---|
| 信源策略 | 全自动抓取；v1 起步 500 精选种子，自动发现扩到上限 5000 |
...
1. 前端查 `daily_cache(device_id, today)`。命中 → 取 `script_json` 跳步骤 5。
2. 未命中 → `POST /api/briefing/generate { device_id, today, length_minutes, topic_ids }`。
3. 服务端：
   a. 拉取用户 topic 对应 sources 的最近 24h articles（去重、按时间 × 源质量排序）
   b. 按 `length_minutes` 估算章节数：基准 200 字/分钟，~1 章/2 分钟，下限 3，上限 12
   c. 调 LLM（structured output）→ `script_json`
   d. 写入 `briefings` 与 `daily_cache`
   e. 返回 `script_json`
4. 前端渲染章节列表。
...
| 项 | 估算 |
|---|---|
| LLM（~1000 调用/天 × 1500 字） | ~$15/天 ≈ $450/月 |
